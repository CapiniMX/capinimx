...
/**
 * 🎨 Generador Interactivo de Imágenes - AprendiendoLuk
 * Guía paso a paso para generar 17 imágenes en Google Gemini
 * Copia prompts al clipboard y abre Gemini automáticamente
 */
...
function openGemini() {
  try {
    ...
    console.log('⚠️  No se pudo abrir Gemini automáticamente. Ve a: https://gemini.google.com/app');
  }
}
...
async function main() {
  ...
  console.log('╔═══════════════════════════════════════════════════════════════════╗');
  console.log('║                                                                   ║');
  console.log('║  🎨 GENERADOR INTERACTIVO DE IMÁGENES - AprendiendoLuk           ║');
  console.log('║                                                                   ║');
  console.log('║  Total imágenes: 17                                              ║');
  console.log('║  Categorías: Screenshots, Mockups, Assets, Website, Infografías  ║');
  ...
  console.log('\n✅ Requisitos:');
  console.log('   • Estar logueado en Google Gemini');
  console.log('   • Tener el navegador Chrome disponible');
  console.log('   • Permisos para descargar en ~/Downloads/\n');

  const ready = await question('¿Estás listo para comenzar? (s/n): ');
  ...
  console.log('\n📱 Abriendo Google Gemini...');
  openGemini();

  await question('✅ Presiona Enter cuando hayas iniciado sesión en Gemini: ');
  ...
  for (let i = 0; i < IMAGES.length; i++) {
    ...
    console.log(`\n╔════════════════════════════════════════════════════════════════╗`);
    console.log(`║  [${String(i + 1).padStart(2)}/${IMAGES.length}] ${image.category}`);
    console.log(`╚════════════════════════════════════════════════════════════════╝\n`);
    ...
    // Copiar prompt
    const copied = copyToClipboard(image.prompt);
    if (copied) {
      console.log('\n✅ Prompt copiado al clipboard');
    } else {
      console.log('\n⚠️  No se pudo copiar al clipboard. Cópialo manualmente.');
    }

    console.log('\n📝 Pasos:');
    console.log('   1. Ve a la ventana de Gemini');
    console.log('   2. Pega el prompt (Cmd+V)');
    console.log('   3. Click en "Generate" o "Create"');
    console.log('   4. Espera a que se genere la imagen (1-2 minutos)');
    console.log(`   5. Descarga con el nombre exacto: ${image.name}`);
    console.log(`   6. Guarda en: ~/Downloads/${image.name}\n`);

    const action = await question(
      '   ¿Imagen descargada? (s=sí, n=saltear, q=salir): '
    );

    if (action.toLowerCase() === 's') {
      console.log(`   ✅ ${image.name} completada\n`);
      completed++;
    } else if (action.toLowerCase() === 'q') {
      console.log('\n⚠️  Generación interrumpida por el usuario');
      break;
    } else {
      console.log(`   ⏭️  ${image.name} saltada\n`);
      skipped++;
    }
  }
  ...
  console.log(`   Saltadas: ${skipped}/${IMAGES.length}`);
  console.log(`\n📁 Todas en: ~/Downloads/\n`);
